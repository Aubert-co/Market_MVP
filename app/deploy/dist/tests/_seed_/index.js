...
async function seedDatabase() {
    const password1 = await bcrypt_1.default.hash("12345678", 10);
    const password2 = await bcrypt_1.default.hash("1234beme", 10);
    const usersData = [
    ...
    await prisma_1.prisma.store.createMany({
        data: storesFormatted
    });
    const dbStores = await prisma_1.prisma.store.findMany();
